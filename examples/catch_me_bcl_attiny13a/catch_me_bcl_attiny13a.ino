/*
 * Catch me bi-color for ATtiny13A
 *  
 ...
 *  
 * Tested on ATtiny13A running at 1.2MHz.
...
const byte OCR0A_initial_value = 4;      // Inital value for OCR0A to trigger interrupt every  3.41ms
const byte OCR0B_initial_value = 38;     // Inital value for OCR0B to trigger interrupt every 32.43ms
...
*  focusing on the flow of current between pairs of ports.
    - 3:2 Bits: These bits determine which port should be set to a high state (active).
    - 1:0 Bits: These bits determine which port should be set to a low state (inactive).
   -------------------------------------------------------------------------------------
*/
...
const byte duration[levels] = {
  139, 122, 107, 94, 83, 73, 64,
  56, 49, 43, 38, 34, 30, 27,
  24, 21, 19, 17, 15, 13, 11
};
...
void setup() {
  ...
  TIMSK0 |= (1 << OCIE0A) | (1 << OCIE0B); // Enable Counter Output Compare Match A and B Interrupt
